...
int index_add(Index *index, const char *path) {
    ...
    void *data = malloc(file_len == 0 ? 1 : file_len);
    if (!data) {
        fclose(f);
        return -1;
    }

    if (file_len > 0 && fread(data, 1, file_len, f) != file_len) {
        free(data);
        fclose(f);
        return -1;
    }
    ...
    ObjectID blob_id;
    if (object_write(OBJ_BLOB, data, file_len, &blob_id) != 0) {
        free(data);
        return -1;
    }
    free(data);
